...
echo "== build crown-index + crown-relay (local profile) and start the replica"
# No cursor seed: Crown-Index does not poll — the book is filled by paid pushes
# through crown-relay (this run pushes each donate/claim signature below).
...
# ordinary call (no cycles), and it forwards to crown-index with cycles — the
# same path the platform uses. Allowlisted to this run's dfx identity.
...
# Push a signature; a pre-finality "not found" is transient, so callers retry.
...
A_CLAIM_TX=$(driver claim "$SOL_RPC_URL" "$SOL_DONOR_KEYPAIR" "$A" 0 "$SIG_A" "$RESOLVER" 2>&1 | sed -n 's/^tx: //p')
echo "claim(A,0) tx: $A_CLAIM_TX"
